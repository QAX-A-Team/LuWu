...
from app.models.config import C2Profile
from app.models.config import Isp
from app.models.config import IspApiKey
from app.models.config import IspType
from app.models.config import SiteTemplate
...
class CrudIsp(CrudConfig):
    def get_available_api_token(self, db_session: Session, isp_type=IspType.domain, isp_provider=None) -> Isp:
        isp_obj = db_session.query(self.model).filter_by(type=isp_type, provider=isp_provider).first()
    ...
    def get_specific_isp_list(self, db_session: Session, *, isp_type: IspType, isp_provider=None) -> Query:
        query_condition = {
            'type': isp_type
        }
        if isp_provider:
    ...
    def update(self, db_session: Session, *, obj_id: int, obj_in: UpdateSchemaType) -> ModelType:
        db_obj = self.get(db_session, obj_id)
        super().update(db_session=db_session, obj_id=obj_id, obj_in=obj_in)
        crud_api_key.update(db_session=db_session, obj_id=db_obj.isp_api_key.id, obj_in=obj_in)
...
class CrudSiteTemplate(CrudConfig):
    def create_site_template(self, db_session: Session, site_template_data: dict) -> ModelType:
    ...
    def update_site_template(self, db_session: Session, template_id: int, **site_template_data) -> ModelType:
...
class CrudSshConfig(CrudConfig):
    ...
    def create_config(self, db_session: Session) -> ModelType:
        ...
        ssh_config_obj = self.model(private_key=private_key, public_key=public_key)
