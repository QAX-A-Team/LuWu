...
class IspApiKey(BaseModel):
    ...
    isp = orm.relationship(
        ...
        uselist=False,
        backref=orm.backref('isp_api_key', single_parent=True, cascade="all, delete-orphan")
    )
...
class Isp(BaseModel):
    ...
    def __init__(self, type, provider, api_key, is_test, remark, **kwargs):
        self.type = type
        self.provider = provider
        self.isp_api_key = IspApiKey(api_key=api_key, is_test=is_test)
        self.remark = remark
    ...
    def isp_instance(self) -> Union[Type[BaseIsp], None]:
        return self.isp_lib(self.api_key, is_test=self.is_test) if self.isp_lib else None
