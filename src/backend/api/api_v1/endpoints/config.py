...
from api.utils.query import C2PaginationQuery
from api.utils.query import DomainIspPaginationQuery
from api.utils.query import PaginationQuery
from api.utils.query import VpsIspPaginationQuery
from crud.crud_config import crud_c2
from crud.crud_config import crud_isp
from crud.crud_config import crud_site_template
...
from schemas.base import TaskResponse
from schemas.config import C2PaginationResponse
from schemas.config import C2ProfileCreate
from schemas.config import C2ProfileCreateResponse
from schemas.config import C2ProfileItemResponse
from schemas.config import C2ProfileUpdate
from schemas.config import DomainIspCreate
from schemas.config import IspAvailableResponse
from schemas.config import IspItemResponse
from schemas.config import IspPaginationQuerySchema
from schemas.config import IspPaginationResponse
from schemas.config import IspUpdate
from schemas.config import SiteTemplateItemResponse
from schemas.config import SiteTemplatePaginationResponse
from schemas.config import SiteTemplateUpdateSchema
from schemas.config import SshConfigResponse
from schemas.config import VpsIspCreate
...
@router.post(
    "/isp/vps", status_code=status.HTTP_201_CREATED, response_model=IspItemResponse
)
def create_vps_isp_profile(
    db: Session = Depends(get_db), *, vps_isp_profile: VpsIspCreate,
):
...
@router.post(
    "/isp/domain", status_code=status.HTTP_201_CREATED, response_model=IspItemResponse
)
def create_domain_isp_profile(
    db: Session = Depends(get_db), *, domain_isp_profile: DomainIspCreate,
):
...
def update_isp_profile(
    db: Session = Depends(get_db), *, isp_id: int, isp_profile: IspUpdate,
):
...
def delete_isp_profile(
    db: Session = Depends(get_db), *, isp_id: int,
):
    ...
    if relation_data_exists:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="exists relation data",
        )
    delete_status = crud_isp.remove(db_session=db, id=isp_id)
    result = {"result": dict(status=delete_status)}
    return result
...
def get_c2_profile_detail(db: Session = Depends(get_db), *, profile_id: int):
...
def update_c2_profile(
    db: Session = Depends(get_db), *, profile_id: int, c2_profile: C2ProfileUpdate,
):
...
def delete_c2_profile(db: Session = Depends(get_db), *, profile_id: int):
    delete_status = crud_c2.remove(db_session=db, id=profile_id)
    return {"result": dict(status=delete_status)}
...
async def create_site_template(
    ...
    created_data = crud_site_template.create_site_template(db, site_template_profile)
    return dict(result=created_data)


@router.put(
    "/template/site/{site_template_id}", response_model=BaseCrudStatusResponse,
)
async def update_site_template(
    db: Session = Depends(get_db), *, site_template_profile: SiteTemplateUpdateSchema
):
    ...
    update_status = {"status": bool(update_result)}
...
async def delete_site_template(db: Session = Depends(get_db), *, site_template_id: int):
    delete_result = {
